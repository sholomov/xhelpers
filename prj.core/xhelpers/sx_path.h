...
public:
  ...
  bool isNetworkPath() const;                                       //!< Сетевой пути. true если путь сетевой
...
inline xpath& xpath::getTempPath(void)
{
  ...
  *this = buf;
  ...
  checkSlash();
  (*this) += "tmp";
  ar::Filesystem::ensureFolder(*this);
#endif
  return *this;         
}
...
#ifdef WIN32
  if (!*buf) strcpy(buf, ".\\");
  char* lpDummyFilePart=NULL;
  ::GetFullPathName(buf, sizeof(buf), buf, &lpDummyFilePart);
#else
  realpath(buf, buf);
#endif
...
//!< Сетевой пути. true если путь сетевой
