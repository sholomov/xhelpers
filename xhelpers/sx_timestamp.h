...
namespace sx
{
  ...
  public:
    ...
    timestamp operator+(const timestamp& dtime) const;    //!< Operator for adding two timestamp variables 
    timestamp operator-(const timestamp& dtime) const;    //!< Operator for subtracting two timestamp variables 
  ...
  {
    // autoremove ending zeroes from tv_nsec
    for (int precision = 9; precision>=0; precision--)
    {
      if(tv_nsec % long(pow(10,9-precision)+0.5) != 0)
  ...
  inline std::string timestamp::toString(int precision) const
  {
    ...
    // Negative is "-" concatenated with absolute value
    if(isNegative())
      return std::string("-") + (-timestamp(*this)).toString(precision);

    tm* tmLocal = localtime( &tv_sec );
    char dateStr[20] = {0}; char dateStrN[36] = {0};
  ...
  inline timestamp& timestamp::operator-=(const timestamp& dtime)
  {
    // operator returns absolute value
    if(*this < dtime)
      return *this = -(timestamp(dtime) -= *this);
  ...
  inline timestamp& timestamp::operator+=(double dsec)
  {
    ...
    decsec += 1e-10 * (decsec>0 ? 1 : -1); // round nano seconds
  ...
  //!< Operator for adding two timestamp variables 
  inline timestamp timestamp::operator+(const timestamp& dtime) const
  {
    return timestamp(*this) += dtime;
  }

  //!< Operator for subtracting two timestamp variables 
  inline timestamp timestamp::operator-(const timestamp& dtime) const
  {
    return timestamp(*this) -= dtime;
  }

  //!< Negative operator val = - val
  inline timestamp timestamp::operator-()
  {
    return timestamp(-tv_sec, -tv_nsec);
  }

  //!< Returns absolute value
  inline timestamp timestamp::abs()
  {
    return isNegative() ? -(*this) : *this;
  }


  //!< Equal, true if the timestamp is equal to dtime
  inline bool timestamp::operator==(const timestamp& dtime) const
  {
    return (tv_sec == dtime.tv_sec) && (tv_nsec == dtime.tv_nsec);
  }

  //!< Less, true if the timestamp is less than dtime
  inline bool timestamp::operator<(const timestamp& dtime) const
  {
    if(tv_sec < dtime.tv_sec)
      return true;
    if(tv_sec == dtime.tv_sec && tv_nsec < dtime.tv_nsec)
      return true;
    return false;
  }

  //!< Greater, true if the timestamp is greater than dtime
  inline bool timestamp::operator>(const timestamp& dtime) const
  {
    if(tv_sec > dtime.tv_sec)
      return true;
  ...
  //!< Not greater, true if the timestamp is less or equal to dtime
  inline bool timestamp::operator<=(const timestamp& dtime) const
  {
    return (*this < dtime) || (*this == dtime);
  }

  //!< Not less, true if the timestamp is greater or equal to dtime
  inline bool timestamp::operator>=(const timestamp& dtime) const
  {
    return (*this > dtime) || (*this == dtime);
  }
